...
// TP configuration: pips before max, dollar after max
input double         InpAvgTPPipsBeforeMax       = 10.0;           // Average TP in pips before max trades
input double         InpDollarTPAfterMax         = 20.0;           // Dollar TP after reaching max trades

// Risk management: Max Drawdown
input double         InpMaxDDDollar              = 100.0;          // Max DD in $ (0 = disabled)
...
// Get total floating profit/loss from all EA trades (both sides)
...
// Close all trades from this EA (both buy and sell)
void CloseAllOrders()
{
...
void TryCloseByTargets()
{
	...
	if(sellCount > 0)
	{
		...
		// TP logic: use pips-based TP before max trades, dollar-based TP after max trades
		if(sellCount >= InpMaxTrades)
		{
			// After max trades: close when profit reaches dollar target
			double sellProfit = SideTotalProfit(false);
			if(sellProfit >= InpDollarTPAfterMax)
			{
				CloseAllSideOrders(false);
				ResetSideState(false);
				sellCount = 0;
		...
		{
			// Before max trades: close when average profit in pips is reached
			double totLots = 0.0;
			...
			if(vwap > 0.0)
			{
				RefreshRates();
				double ask = Ask;
				double pipsGain = (vwap - ask) / PipSize();
				if(pipsGain >= InpAvgTPPipsBeforeMax)
				{
					CloseAllSideOrders(false);
					ResetSideState(false);
				}
	...
	if(buyCount > 0)
	{
		...
		{
			// After max trades: close when profit reaches dollar target
			double buyProfit = SideTotalProfit(true);
			if(buyProfit >= InpDollarTPAfterMax)
			{
				CloseAllSideOrders(true);
				ResetSideState(true);
				buyCount = 0;
		...
		else if(buyCount > 0)
		{
			...
			if(vwapB > 0.0)
			{
				RefreshRates();
				double bid = Bid;
				double pipsGainB = (bid - vwapB) / PipSize();
				if(pipsGainB >= InpAvgTPPipsBeforeMax)
				{
					CloseAllSideOrders(true);
					ResetSideState(true);
				}
...
void OnTick()
{
	// Max DD check first - close all trades if floating loss exceeds threshold
	if(InpMaxDDDollar > 0.0)
	{
		double floatingPL = GetTotalFloatingProfit();
		if(floatingPL <= -InpMaxDDDollar)
		{
			CloseAllOrders();
			return; // Exit tick processing after DD close
		}
	}
